...
test:
	...
	R --slave -e 'Sys.setenv(NOT_CRAN="true"); library(testthat); setwd(file.path(.libPaths()[1], "rehash", "tests")); system.time(test_check("rehash", filter="${file}", reporter=ifelse(nchar("${r}"), "${r}", "summary")))'
...
check: build
	-R CMD check --as-cran rehash_$(VERSION).tar.gz
	rm -rf rehash.Rcheck/
